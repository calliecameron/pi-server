...
class Test02PiCore:
    ...
    def test_01_vars(self, hostname: str, hosts: Dict[str, Host], masks: Dict[str, str]) -> None:
        host = hosts[hostname]
        assert host.file('/etc/pi-server/lan-network-addr').content_string.strip() == masks['lan' + host_number(hostname)]
        assert host.file('/etc/pi-server/vpn-network-addr').content_string.strip() == masks['pi%s_vpn' % host_number(hostname)]
